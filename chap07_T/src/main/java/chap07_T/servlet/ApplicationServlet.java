...
import chap07_T.util.UriParser;
import chap07_T.webprocess.BreadAddFormWebProcess;
import chap07_T.webprocess.BreadAddWebProcess;
import chap07_T.webprocess.BreadListWebProcess;
import chap07_T.webprocess.BreadRemoveFormWebProcess;
import chap07_T.webprocess.BreadRemoveWebProcess;
import chap07_T.webprocess.BreadUpdateFormWebProcess;
import chap07_T.webprocess.BreadUpdateWebProcess;
import chap07_T.webprocess.IndexWebProcess;
import chap07_T.webprocess.WebProcess;
import chap07_T.webprocess.dbTestWebProcess;
...
public class ApplicationServlet extends HttpServlet {
	...
	static {
		//(1) 전달받은 URI(cmd)에 따라 알맞은 웹 프로세스를 꺼내줄 수 있는 맵을 생성해두면 if문 사용 안해도 된다 (커맨드패턴)
		...
		// add파일에서 빵을 추가한 후 계속 그 파일에 있기 위해서 form action=./add로 놓는다면
		// UriParser클래스에서 내 방식만의 커멘드를 만들어준다
		uriMapping.put("GET::/dbtest/bread/delete", new BreadRemoveFormWebProcess());
		uriMapping.put("POST::/dbtest/bread/delete", new BreadRemoveWebProcess());
...
//	}
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		WebProcess webProcess = uriMapping.get(cmd);
		if (webProcess != null) {
		    String nextPage = webProcess.process(req);
